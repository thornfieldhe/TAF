...
namespace TAF.TAFExpression
{
    ...
    internal class ExpressionWriterSql : ExpressionVisitor
    {
        ...
        ExpSqlType bizRead = ExpSqlType.aiWhere;
        ...
        private static string Write(TextWriter writer, Expression expression, ExpSqlType bizSql)
        {
            ...
            var bizR = new ExpressionWriterSql(writer) { bizRead = bizSql };
            bizR.Visit(expression);
            string result;
            switch (bizSql)
            {
                case ExpSqlType.aiOrder:
                    result = Regex.Replace(bizR.aiOrdeRsult, @",\s?$", "");
                    return result;

                case ExpSqlType.aiWhere:
        ...
        protected void Write(string text)
        {
            ...
            {
                case ExpSqlType.aiOrder:
                    this.aiOrdeRsult = this.aiOrdeRsult + text;
                    break;
                case ExpSqlType.aiWhere:
                    this.aiWhereResult = this.aiWhereResult + text;
                    break;
            }
        ...
        protected override Expression VisitLambda(LambdaExpression lambda)
        {
            ...
            {
                if (((MemberExpression)lambda.Body).Type == typeof(bool) && (this.bizRead == ExpSqlType.aiWhere))
                {
        ...
        protected override Expression VisitMethodCall(MethodCallExpression m)
        {
            ...
            //}
            var bizname = m.Method.Name.ToLower();
            ...
            {
                case "where":
                    if (this.bizRead == ExpSqlType.aiWhere)
                    {
                        this.AiWhereTime = this.AiWhereTime + 1;
                    }
                    else
                    {
                ...
                case "thenby":
                    if (this.bizRead == ExpSqlType.aiOrder)
                    {
                        this.AiOrderTime = this.AiOrderTime + 1;
                    }
                    else
                    {
                        this.Visit(m.Arguments[0]);
            ...
            switch (bizname)
            {
                case "orderbydescending":
                case "thenbydescending":
                    this.aiOrdeRsult = this.aiOrdeRsult + " Desc";
                    break;

            }
            if (this.bizRead == ExpSqlType.aiOrder)
            {
                this.aiOrdeRsult = this.aiOrdeRsult + ",";
            }
            else
            {
                this.aiWhereResult = this.aiWhereResult + " and ";
